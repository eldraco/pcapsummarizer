...
# Files:{
# capinfos: {
# Description: from capinfos tool
# }
# dnstop: {
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--createfrom', help='Create a metadata json from a README.md file passed here.', required=False)
    ...
    if args.createfrom:
        ...
        # From README.md file
        out = subprocess.Popen(["grep", "-i", "Description:", args.createfrom], stdout=subprocess.PIPE)
        description = ' '.join(out.communicate()[0].decode("utf-8").split()[2:])
        if description:
            metadata['Description'] = description

        # Get dataset name if possible
        out = subprocess.Popen(["grep", "-i", "Generic Dataset name", args.createfrom], stdout=subprocess.PIPE)
        name = ' '.join(out.communicate()[0].decode("utf-8").split()[4:])
        if name:
            metadata['Name'] = name

        # If it was malware, get malware name
        out = subprocess.Popen(["grep", "-i", "Probable Malware Name", args.createfrom], stdout=subprocess.PIPE)
        ...
        # From capinfo file
        capinfosfile = os.path.isfile(os.path.join('.', '*'))
        starttime = ''
        endtime = ''
